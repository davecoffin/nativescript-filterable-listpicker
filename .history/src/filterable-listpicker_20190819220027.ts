...
let unfilteredSource: ObservableArray<SourcesDataItems> = new ObservableArray<SourcesDataItems>();
...
export const sourceProperty = new Property<
  ...
  ObservableArray<SourcesDataItems>
>({
  name: "source",
  defaultValue: undefined,
  affectsLayout: true,
  valueChanged: (target, oldValue, newValue) => {
    if (!filtering) {
      ...
      newValue.forEach(element => {
        ...
        unfilteredSource.push(new SourcesDataItems(element.title, element.image, element.description));
      });
    }
  }
});
...
export class FilterableListpicker extends GridLayout {
  ...
  private _searchFilterFn(data: ObservableArray<SourcesDataItems>) {
    filtering = true;
    let i = 0;
    this.source = unfilteredSource.filter(item => {
      ...
      if (item.title) {
        return (
          item.title.toLowerCase().indexOf(data.get("title").toLowerCase()) !== -1
        );
      } else {
