...
export class FilterableListpicker extends GridLayout {
  ...
  private _searchFilterFn(data: any) {
    filtering = true;
    let i = 0;
    this.source = unfilteredSource.filter((item, index) => {
            console.log(index)
        if(i === 0) {
        }
      if (unfilteredSource.getItem(index).title) {
        return (
          unfilteredSource.getItem(index).title.toLowerCase().indexOf(data.value.toLowerCase()) !== -1
        );
      } else {
